"use strict";function B(n,t){$4.innerHTML=n;Q($4);setTimeout(function(){R($4)},t)}function C(n){if(!A.e)if(n===13){if(A.X===5){if(E()===-1){B("Mot inconnu !",1e3);return}D();A.Y++;A.X=0;A.Y===1&&L(0)}}else n===8?A.X>0&&(A.X--,A.g[A.Y][A.X]="",A.h[A.Y][A.X]=γ,F(A.Y)):n>=65&&n<=90&&A.X<5&&(A.g[A.Y][A.X]=String.fromCharCode(n),A.h[A.Y][A.X]=γ,A.X++,F(A.Y,A.Y*5+A.X-1))}function D(){for(let n=0;n<5;n++)A.h[A.Y][n]=δ;let n="",t=A.c.split("");for(let i=0;i<5;i++)n=A.g[A.Y][i],n===t[i]&&(A.h[A.Y][i]=ζ,t[i]="-");for(let i=0;i<5;i++){n=A.g[A.Y][i];const r=t.indexOf(n);r!==-1&&A.h[A.Y][i]===δ&&(A.h[A.Y][i]=ε,t[r]="-")}const i=""+location;i.startsWith("f")||(n=i===A.f?"":location=J(Z));A.e=A.d===A.c||A.Y===5;F(A.Y+1);G()}function E(){A.d=A.g[A.Y].join("");return Z.findIndex(n=>n===A.d)}function F(n,t){let i=0,u=A.Y*5+A.X;for(let e=0;e<6;e++)for(let o=0;o<5;o++){if($1[i].textContent=A.g[e][o],$1[i].className="case "+α[A.h[e][o]],e===n&&A.e===!1){if(A.g[e][o]==="")for(let t=0;t<n;t++)if(A.h[t][o]===ζ){$1[i].textContent=A.g[t][o];$1[i].className="case info";break}i===t&&($1[i].className+=" pop");i===u&&($1[i].innerHTML+="<span class='curseur'>_<\/span>")}i++;var f=A.g[e][o],r=A.h[e][o];for(let n=0;n<$2.length;n++)if($2[n].textContent===f){$2[n].tabIndex<r&&($2[n].tabIndex=r,$2[n].className=α[r]);break}}H()}function G(){if(A.e){const n=A.d===A.c,r=n?"Bravo !":`Solution : ${A.c}`;document.querySelector(".resultats h3").textContent=r;var t=n?1+A.Y:"X",i=`(bêta) LeMOT ${A.a} ${t}/6
