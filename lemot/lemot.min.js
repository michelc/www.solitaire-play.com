"use strict";function B(n,t){$4.innerHTML=n;$4.style.display="block";setTimeout(function(){$4.style.display="none"},t)}function C(n){if(!A.e)if(n===13){if(A.X===5){if(E()===-1){B("Mot inconnu !",1e3);return}D();A.Y++;A.X=0}}else n===8?A.X>0&&(A.X--,A.g[A.Y][A.X]="",A.h[A.Y][A.X]=γ,F()):n>=65&&n<=90&&A.X<5&&(A.g[A.Y][A.X]=String.fromCharCode(n),A.h[A.Y][A.X]=γ,A.X++,F())}function D(){for(let n=0;n<5;n++)A.h[A.Y][n]=δ;let n="",t=A.c.split("");for(let i=0;i<5;i++)n=A.g[A.Y][i],n===t[i]&&(A.h[A.Y][i]=ζ,t[i]="-");for(let i=0;i<5;i++){n=A.g[A.Y][i];const r=t.indexOf(n);r!==-1&&A.h[A.Y][i]===δ&&(A.h[A.Y][i]=ε,t[r]="-")}const i=""+location;i.startsWith("f")||(n=i===A.f?"":location=J(Z));A.e=A.d===A.c||A.Y===5;F();G()}function E(){A.d=A.g[A.Y].join("");return Z.findIndex(n=>n===A.d)}function F(){var n,t,f,r,i;let u=0;for(n=0;n<6;n++)for(t=0;t<5;t++)for($1[u].textContent=A.g[n][t],$1[u].className="case "+α[A.h[n][t]],u++,f=A.g[n][t],r=A.h[n][t],i=0;i<$2.length;i++)if($2[i].textContent===f){$2[i].tabIndex<r&&($2[i].tabIndex=r,$2[i].className=α[r]);break}H()}function G(){var n,t,i;if(A.e){for(n=`(bêta) LeMOT ${A.a} ${1+A.Y}/6
